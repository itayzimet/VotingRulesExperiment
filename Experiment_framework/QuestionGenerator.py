...
def execute_function(_num_winners: int, _num_candidates: int, _num_voters: int, _budget: int,
    ...
    if type(model) is list:
        expression = model
        question = [
            eval(expr.__str__(), {'winners': _num_winners, 'candidates': _num_candidates, 'voters': _num_voters,
                                  'budget': _budget}) for expr in expression]
    else:
        question = model(torch.tensor([_num_winners, _num_candidates, _num_voters, _budget],
                                      dtype = torch.float32).unsqueeze(0)).squeeze().tolist()
