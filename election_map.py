...
def generate_election_map(
    ...
    if compute_distances:
        experiment.compute_distances(distance_id = distance_id, num_processes = 10)
    # %% compute feature
    if compute_feature:
        compute_features(experiment)
    # %% embed 2d and print map
    if embed:
        experiment.embed_2d(embedding_id = 'fr', left='UN', right='ID', up='AN')
    if print_map:
...
def print_maps(experiment):
    ...
    experiment.print_map_2d(saveas = 'map.svg', textual = ['ID', 'UN', 'AN', 'ST'],
                            legend_pos = (0.5, 0), show = False, return_map = True)
    omit = []
    for i in range(0, 19):
    ...
    print_map = partial(experiment.print_map_2d_colored_by_feature, cmap = cmap,
                        textual = ['ID', 'UN', 'AN', 'ST'], omit = omit, strech=[0,100])
    for rule in voting_rules:
        print_map(feature_id = rule.__str__(), saveas = f'map_{rule.__str__()}.svg')
...
def mapof_feature(election: mapof.OrdinalElection, rule: VotingRuleConstrained) -> dict:
    ...
    num_questions = list(range(1, 800000, 10000))
