...
from Experiment_framework.ai_framework import (crossover, evaluate_chromosome, evaluate_function, initialize_population,
                                               mutate_function, test_best_function, tournament_selection)
...
def genetic_algorithm(pop_size, generations, tournament_size, crossover_rate, mutation_rate):
	...
	population_fitness = []
	for chromosome in tqdm(population, desc = 'Evaluating initial population', leave = True):
	...
	gens = trange(generations, desc = 'Running genetic algorithm', postfix = f"Best fitness: "
	f"{best_fitness % 1e6:.2f})",
	              leave = True)
